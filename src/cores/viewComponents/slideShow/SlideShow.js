...
export default class Carousel extends React.Component<CarouselProps, CarouselState> {
     ...
     pageAnimation: Animated.CompositeAnimation;
     panResponder: PanResponderInstance;
     currentIndex: number = 0;
     panStartIndex: number = 0;
     panOffsetFactor: number = 0;
    ...
    constructor(props) {
     ...
     computePanOffset(g: PanResponderGestureState) {
     ...
     endPanResponder(g: PanResponderGestureState) {
        if (this.props.autoplay) {
            this.startAutoPlay();
        }
     ...
     gotoNextPage(animated: boolean = true) {
     ...
     gotoPage(index: number, animated: boolean = true, cb = () => { }) {
        const childrenNum = this.getChildrenNum();
        if (childrenNum <= 1) {
            return cb();
        }
